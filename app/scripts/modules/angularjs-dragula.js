...
function dragula (initialContainers, options) {
  ...
  if (o.canMoveX === void 0) { o.canMoveX = true; }
  if (o.canMoveY === void 0) { o.canMoveY = true; }
  ...
  function grab (e) {
    ...
    _grabbed = context;
    var rect = context.item.getBoundingClientRect();
    _position = {x:rect.x,y:rect.y};
    eventualMovements();
    if (e.type === 'mousedown') {
  ...
  function drag (e) {
    ...
    _mirror.style.left = o.canMoveX ? (x + 'px') : (_position.x + 'px');
    _mirror.style.top = o.canMoveY ? (y + 'px') : (_position.y + 'px');
    ...
    //         _currentSibling = reference;
    //         lock = true;
    //         scrollItem(item,reference,dropTarget);
    //
    //     }
    //     function moved (type) { drake.emit(type, item, _lastDropTarget, _source); }
    //     function over () { if (changed) { moved('over'); } }
  ...
  // function scrollItem(item,reference, dropTarget) {
  //     if (!lock)return;
  //     lock = false;
  //     var ele;
  //     var y;
  ...
  //         ele = dropTarget.children[dropTarget.children.length-1];
  //
  //     }
  //     else if (reference!=null && reference !== nextEl(item)) {
  //         y = item.offsetHeight * -1;
  //         ele = $(item).next()[0];
  //     }
  //     ele.style.transform = `translateY(${y}px)`;
  //     ele.style.transition = "transform 0.3s ease 0s";
  //     setTimeout(()=>{
  //         lock = true;
  //         ele.style.transform = '';
